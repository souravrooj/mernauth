...
class UserController {
      ...
      if (name && email && password && password_confirmation && tc) {
        if (password === password_confirmation) {
          try {
            ...
            //Generate Token
            const token = jwt.sign({ userID: saved_user }, process.env.JWT_SECRET_KEY, { expiresIn: '5d' })
  ...
  static changeUserPassword = async (req, res) => {
    ...
    if (password && password_confirmation) {
      if (password !== password_confirmation) {
        res.send({ "status": "failed", "message": "Password and Confirm Password aren't match" })
      } else {
        const salt = await bcrypt.genSalt(10)
  ...
  static sendPasswordReset = async (req, res) => {
    ...
    if (email) {
      ...
      res.send({ "status": "failed", "message": "Email Fields are require..." })
    }
  }
    ...
    try {
      ...
      if (password && password_confirmation) {
        if (password === password_confirmation) {
          ...
          res.send({ "status": "failed", "message": "New Password & New Confirm Password Doesn't match" })
        }
      } else {
